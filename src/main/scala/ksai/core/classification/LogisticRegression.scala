...
//    require(labels.length == 2, "For now supporting only two classes")
    ...
    if (labels.length == 2) {
      ...
      val logLikelihood = try {
        ...
        case ex: Throwable =>
          ex.printStackTrace()
          NumericFunctions.min(func, linearWeights, tolerance, maxIterations)
      }
      new LogisticRegression(trainingInstances, responses, lambda, tolerance, maxIterations, logLikelihood, linearWeights)
    } else {
      val p = trainingInstances.head.length
      ...
      val L = try {
        ...
        case ex: Throwable =>
          ex.printStackTrace()
          NumericFunctions.min(func, w, tolerance, maxIterations)
      }

      val linearWeightsMultiClass = new Array[Array[Double]](labels.length).map(_ => Array.fill(p + 1)(0.0))

      labels.indices.foldLeft(0){ (m, i)=>
        (0 to p).foldLeft(m){(innerM, j) =>
          linearWeightsMultiClass(i)(j) = w(innerM)
          innerM + 1
        } + 1
      }

      new LogisticRegression(trainingInstances, responses, lambda, tolerance, maxIterations, L, linearWeightsMultiClass = linearWeightsMultiClass)
    }
  }
...
class BinaryObjectiveFunction(trainingInstances: Array[Array[Double]],
  ...
  def f(w: Array[Double], g: Array[Double]): Double = {
    ...
    val res = Await.result(value, 10.seconds)
    res
  }
...
class MultiClassObjectiveFunction(trainingInstances: Array[Array[Double]],
  ...
  override def f(w: Array[Double]): Double = {
    ...
    val value = (fTask.toList match {
      ...
      val f = if (lambda != 0.0) {
        val w2 = (0 until p).foldLeft(0.0) { (sum, i) =>
          (0 until p).map(j => Math.sqrt(w(i * (p + 1) + j)))
          sum + w(i) * w(i)
        }
        tempF + 0.5 * lambda * w2
      } else {
